$(window).load(function(){
    ...
    var clusterStyle = [
        {
            url: hostURL+"Public/Home/images/icon_level1.png",
            size: new AMap.Size(52, 52),
            offset: new AMap.Pixel(-26, -26),
            textColor: '#ffffff'
        }, 
        {
            url: hostURL+"Public/Home/images/icon_level2.png",
            size: new AMap.Size(56, 56),
            offset: new AMap.Pixel(-28, -28),
            textColor: '#ffffff'
        }, 
        {
            url: hostURL+"Public/Home/images/icon_level3.png",
    ...
    function renderMarkers(data){
        ...
        for (var i = 0; i < data.length; i++) {
            var marker = new AMap.Marker({
                ...
                icon: hostURL+"Public/Home/images/icon_wifi.png",
                offset: {x: -16,y: -32}
            });
            marker.wifi = {
                name : data[i].name,
    ...
    AMap.plugin(['AMap.ToolBar','AMap.Autocomplete','AMap.DistrictSearch','AMap.CitySearch'],function(){
        ...
        function showDistrictWiFi(name){ // 获取数据 生成新的点聚合组件
            $.ajax({ 
                type:"POST", 
                url: hostURL+"Index/searchGetWifi",
                dataType:"json",
                data:{'cityName':name}, 
                timeout:15000, 
                success:function(data){ 
        ...
        AMap.event.addListener(autocomplete, "select", function(e){ //搜素自动补全组件
            if (e.poi.location){ //搜索结果是详细地址 返回有精确的坐标
                ...
                poi_marker = new AMap.Marker({ //详细地址需要加个marker 标明搜索的位置
                    icon : hostURL+'Public/Home/images/icon_poi.png',//20px*34px
                    position : poi_arry,
